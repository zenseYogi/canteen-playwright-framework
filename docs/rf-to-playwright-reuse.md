...
- ~~**Login/Password coordinate-tap stopgap confirmed still necessary and still accurate**~~ **Superseded (see "WebView debugging enabled" section below)**: `uiautomator dump` shows the Login screen is genuinely `class="android.webkit.WebView"` with `NAF="true"`; `adb shell cat /proc/net/unix | grep webview_devtools` is still empty. `LoginScreen.enterLoginId()`'s exact tap math (`width×0.5, height×0.39`) and `typeViaAdb`'s mechanism were both tested directly against the real screen and work. This was true for build 0.1.73; a later QA-debug build (0.1.76) enables WebView debugging, making the coordinate-tap stopgap unnecessary - see below.
...
## WebView debugging enabled (build 0.1.76) - Login/Password rewritten off the coordinate-tap stopgap

A new build, `nexus-app-qa-debug-220726.apk` (version 0.1.76, package `com.compass.canteen.nexus.test`), was live-verified via `aapt2 dump badging` to be a QA-debug variant (carries the `application-debuggable` flag) with a distinct `applicationId` from the earlier production-style build documented throughout this doc (`com.canteen.nexus`, version 0.1.73). This is the reason the Login/Password coordinate-tap stopgap - confirmed still necessary as recently as the 0.1.73 live-verification session above - is now obsolete for this build.

- **WebView debugging is genuinely on**, not just theoretically enabled by the debuggable flag: confirmed via `adb shell cat /proc/net/unix | grep webview_devtools_remote` returning a live socket, and independently via Chrome DevTools Protocol page inspection over `adb forward tcp:9222 localabstract:webview_devtools_remote_<pid>`.
- **Appium can create a real `WEBVIEW_com.compass.canteen.nexus.test` context** and drive it with standard CSS-id selectors - confirmed by an end-to-end live run, not just a context-list check: Login ID (`#login-id`) -> Continue (`#get-home-realm-details-button`) on the CAS/SSO page (`ssoqas.compassmanager.com`) -> real cross-origin redirect to Microsoft Entra ID / Azure AD (`login.microsoftonline.com`) -> password field `#i0118` -> Sign in `#idSIButton9` -> MFA number-match screen -> a native "Route Setup Required" dialog. That dialog is an account/environment gap (this specific test account has no route assigned in this env) blocking reaching Dashboard for this account specifically - not a locator bug, and not evidence against the WebView flow itself.
- **`screens/login.screen.ts` and `screens/password.screen.ts` were rewritten** to use these real WebView locators via `BaseScreen.switchToWebView()`/`switchToNative()` (both already existed on `BaseScreen`) instead of the old `tapViaAdb`/`typeViaAdb`/fixed-percentage-coordinate stopgap. `PasswordScreen.tapSignIn()` now calls `switchToNative()` at the end, since everything after Sign In (permission dialog, MFA) is native Flutter again, not WebView.
- **Real, newly-observed app behavior**: a native "Login Timed Out" dialog fires if too much wall-clock time elapses during the CAS -> Azure AD handshake (observed when slow manual debugging/retries ate the window). This is a genuine app-level timeout, not a test bug - any real spec/fixture needs to move through Login -> Continue -> Password -> Sign in promptly.
- **Pre-existing latent config bug found and fixed, independent of the WebView work**: `playwright.config.ts`'s global `timeout` was `60_000`, but every login-dependent spec calls `MfaScreen.waitForManualApproval(120_000)` - meaning the overall test timeout was always shorter than the wait it contained, so such tests failed before a human could ever approve the push regardless of app behavior. Raised to `150_000`. This bug has been latent since MFA waiting was written; it only surfaced now because this session was the first time the real end-to-end flow was actually run rather than stopped at the coordinate-tap stage.
- ~~**"Select a day" (Settings > Route setup > change route > confirm > sync > Select a day tiles) is cosmetic/non-functional**: tapping YESTERDAY appeared to just dismiss the sheet back to Today's Dashboard with no change.~~ **Correction: this was wrong, caused by a tap-coordinate measurement error, not a real app limitation.** The taps were made using display-screenshot-scaled pixel coordinates directly on the real device, without multiplying by the 1.2x scale factor needed to convert screenshot pixels to real device pixels - so the taps were landing on the wrong targets, not exercising the actual tiles. Manually re-tested (correct coordinates) and confirmed day-switching works correctly: selecting "Yesterday, Tue 21 Jul" on this account (Miami, FL / Route 010 - shown as "Route 10" on the Dashboard header but "Route 010" in the Route Setup dropdown; a minor display inconsistency, not a bug) correctly loaded a different day's data - "Yesterday, Tue 21 Jul", 4 Deliveries, Market/Coffee LOB tabs showing counts (0/3 Market, 0/1 Coffee), and a Schedule section listing real stops ("Abco Products" - Coffee, "CureLeaf" - Market, "FedEx" - Market) with "Start day" enabled. **Today's route (Wed 22 Jul) has zero scheduled deliveries** (0 Delivery, "Start day" greyed out), which blocks any live Market/Coffee/Vending LOB service-flow testing (`dashboard.clickLocationByPosition`/`openFirstServiceStation` need an actual stop to click into) - **Yesterday's route is the viable day to use for that work going forward** on this account.

## Live verification session (build 0.1.76, Route 103) - Vending P1 test-case scripting

With Route 103 (a Vending-only route, first stop "Aaron's", first machine "61241 - Lg Snacks") reachable via the Route Setup flow, this session live-verified Vending's real screen structure and wrote the first Vending spec against `Optimized_TCs_V_2.0.xlsx`.

- **Vending P1 (Mandatory) test-case counts from the Excel**: 87 total across all layers, 58 UI-layer. By Sub Area: Money ops (29), delivery-Filters (17), delivery-Product delivery (16), delivery-Sort (14), Planogram (3), Removals & Returns-Sort (2), Machine-Full Service (2), After Photos (1), Removals & Returns (1), Removals & Returns-Filter (1), Machine-Wet Machine–Meters (1).
- **Real machine-service navigation confirmed**: tapping a stop's "vending" card (an `android.widget.ImageView`, content-desc `"vending\n3 Service stations"`) expands to per-machine rows (e.g. `"61241 - Lg Snacks\nBreakroom"`, each a plain `android.view.View` with **no "vending" substring** in its content-desc at all). Tapping a machine row opens its service screen with 5 sections: Before Photos, Money Operations, Fills, Removals & Returns, After Photos.
- **Real bug found and fixed in `screens/dashboard.screen.ts`**: `firstServiceStationUnder(lob)` assumed a service station row's own content-desc contains the LOB name - true for single-station LOBs, but Vending machine rows have arbitrary data-driven names with no "vending" substring, so the method matched nothing once a stop has more than one Vending machine. Added `nthServiceStationUnder(lob, position)` / `openNthServiceStation(lob, position)`, which instead walks from the card header (which DOES start with the LOB name) to its Nth following `android.view.View` sibling - with a live-verified **`+1` offset**: the header's immediate first sibling is a non-clickable numeric badge view (content-desc `"0"`), not an actual station row.
- **Vending's "Money Collection" screen** (opened via a machine's "Money Operations" tile) has only **3** `EditText` fields (bag code / Replenishment Bills / Refund amount) vs. Market's 4 (Market also has a separate Coins field) - all positional, no hint/content-desc of their own.
- **Product fills screen's shared header controls confirmed genuinely LOB-agnostic**: `section_header_sort_cta` and `section_header_filter_cta` (both `android.widget.Switch` elements, previously spotted on Market but never used/confirmed). Their real "is active" signal is the **`checked` attribute, not visibility** - live-confirmed: selecting a Sort option (e.g. "A to Z") both re-orders the product list alphabetically and flips `section_header_sort_cta`'s `checked` to `true` (visible green/dot badge state); selecting a Filter category chip (e.g. "CANDY (6)") and tapping "Apply filters" flips `section_header_filter_cta`'s `checked` to `true` and adds a removable "CANDY x" chip above the list, plus enables both "Clear filters" and "Apply filters" (both start disabled with zero chips selected).
- **New, previously-undocumented app behavior**: navigating back from Product fills with an unsaved Sort/Filter selection triggers a native "Save Changes!" confirmation dialog (Discard/Save buttons) - not mentioned anywhere in the RF suite or earlier docs.
- **New shared `BaseScreen` additions** (used across LOBs, not Vending-specific): `sortCta`/`filterCta`/`planogramCta` locators, `isChecked(selector)`, `selectSortOption(optionLabel)`, `isSortActive()`, `selectFilterCategories(categoryLabels[])`, `isFilterActive()`.
- **`screens/vending-service.screen.ts` rewritten** with live-verified locators/methods: `openFills()`, `isProductFillsTitleVisible()`, `openFillsAndContinue()` (kept for compatibility), `openMoneyOperations()`, `isMoneyCollectionScreenVisible()` (title/skipMoneyBag/bagCode/bills/refund), `performMoneyOperations(values)`.
- **`tests/mobile/vending-service.spec.ts` written**, two tests TC-tagged against the Excel rows verified above: test 1 tags `@TC163 @TC164 @TC166 @TC167 @TC212 @TC215 @TC181 @TC183 @TC186` (Product fills load, Sort open/select/active, Filter open/chips-with-count/select-active); test 2 tags `@TC244 @TC246 @TC248` (Money Operations title/all-sections/Skip-Money-Bag-label). Both tests require the account to be on Route 103, reached via the Route Setup flow (`tests/mobile/route-setup.spec.ts`).

### Environment/config notes for running this build

- **Chromedriver must be pre-cached locally** - Appium needs a ChromeDriver matching the emulator's system WebView Chrome version (109.0.5414) to drive the `WEBVIEW_*` context at all. `appium:chromedriverAutodownload: true` alone did not fetch it. Had to manually download `chromedriver.storage.googleapis.com/109.0.5414.74/chromedriver_mac_arm64.zip` and place it under `~/.appium/node_modules/appium-uiautomator2-driver/node_modules/appium-chromedriver/chromedriver/mac/109.0.5414.74`. This is local machine setup, not a repo file - on a fresh machine, WEBVIEW context switching will fail with "No Chromedriver found" until this is done.
- **`config/mobile.config.ts`'s defaults don't match this build**: it falls back to `com.canteen.nexus`/`.MainActivity` (`process.env.APP_PACKAGE ?? 'com.canteen.nexus'`) when env vars aren't set, which is the older production-style package, not this new test build's real installed package. `.env` was updated to add `APP_PACKAGE=com.compass.canteen.nexus.test` and `APP_ACTIVITY=com.compass.canteen.nexus.MainActivity` to override the defaults through that existing fallback. Note the activity class itself did NOT get a `.test` suffix - only the applicationId did.
